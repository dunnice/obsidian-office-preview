...
- 📽️ **PowerPoint 演示文稿 (`.pptx` / `.ppt`) 预览**
  - **双模式自由切换**：支持连续文档流式的**列表模式 (List)** 与沉浸式逐页展示的**单页模式 (Slide)**。
  - **翻页与快捷键导航**：单页模式下支持上一页/下一页按钮、键盘左右箭头 (`←` / `→`)、`PageUp` / `PageDown` 以及空格键快速翻页。
  - **自适应视口与动态缩放**：内置 `ResizeObserver` 自动根据容器宽度自适应居中适配，支持自由缩放 (`+` / `-` / `自适应`)。
  - **🖥️ 一键全屏演示**：点击全屏按钮即可进入沉浸式全黑背景演示模式。
  - **🔍 幻灯片全文检索**：支持快捷键 `Cmd+F` 检索所有幻灯片文本内容，实时高亮与平滑定位跳转。
  - **智能识别旧版 `.ppt`**：自动识别 OLE2 二进制 `.ppt` 文件，提供专有引导界面与“一键在系统默认应用中打开”操作。
